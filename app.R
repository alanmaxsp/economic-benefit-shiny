...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      numericInput("L","Annual survival of rams (s)",NULL,step=0.01),
      numericInput("f","Mating ratio - ewes per ram (f)",NULL),
      numericInput("p","Lambs weaned per ewe mated in T2 and T3 (p)",NULL,step=0.01),
      numericInput("m1","Survival between weaning and first shearing (s1)",NULL,step=0.01),
      numericInput("m2","Annual survival after first shearing (s2)",NULL,step=0.01),
...
server <- function(input, output, session) {
  ...
  calcular <- function(c, b1, b2, L, f, p, m1, m2, u1, u2,
                       ecd, eca, ecm, ece,
    ...
    wm <- rep(0, H); hc <- tt[1]/p
    for(i in 3:(u1+2)) wm[i] <- hc * tt[i-2]
    ...
    if(T > 1) for(i in 2:T) cosT1[i] <- costoa * fr[i-1]
    CB1 <- sum(cosT1)
    cosT2 <- rep(0, H); cosT2[1] <- ram1 * c * b1
    if(T > 1) for(i in 2:T) cosT2[i] <- ram1 * c * b1 * fr[i-1]
    CB2 <- sum(cosT2)
    cosT3 <- rep(0, H)
    for(i in 3:(T+2)) if(i <= H) cosT3[i] <- ram2 * noveE2 * b2 * fr[i-1]
    CB3 <- sum(cosT3)
    costot <- CB1 + CB2 + CB3
    ...
    # Convierte a carácter solo la última fila, agregando "%"
    ...
    tbl <- data.frame(
      Parameter = c(
        ...
        "Return to investment (stud costs)",
    ...
    # # ---- Tabla resultados ----
    # tbl <- data.frame(
    #   Parameter = c("IA2","IA3","CB1","CB2","CB3","Net Benefit"),
    #   Value     = round(c(IA2, IA3, CB1, CB2, CB3, IA2+IA3-CB1),2)
    # )
    
    # ---- Time series para gráfico ----
    ...
    # Usar nombres simples sin signos
    df_time <- data.frame(
      Year      = 0:H,
      T2        = Benefit_in_T2,
